...
try {
  if (fs.existsSync(resolve(appPath))) {
    fs.unlinkSync(resolve(appPath));
  }
} catch (err) {}
...
if (!fs.existsSync(resolve(appPath))) {
  ...
  console.log('  ', resolve(commonPath), '    - to - ');
  console.log('  ', resolve(appPath));
  console.log(
...
function createRootSymLink() {
  ...
  var p1 = resolve(AppPath + '/' + appPath);
  var p2 = resolve(AppPath + '/' + commonPath);
  if (debugging) {
...
function createSymLink() {
  if (debugging) {
    console.log('Attempting to Symlink', commonPath, appPath);
  }
  fs.symlinkSync(resolve(commonPath), resolve(appPath), 'junction');
}
